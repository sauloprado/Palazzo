...
function optionsframework_tabs() {
	...
	foreach ($options as $value) {
		...
		if ($value['type'] == "heading") {
			...
			$menu .= '<a id="'.  esc_attr( $jquery_click_hook ) . '-tab" class="nav-tab" title="' . esc_attr( $value['name'] ) . '" href="' . esc_attr( '#'.  $jquery_click_hook ) . '">' . esc_html( $value['name'] ) . '<span class=" ' . $value['class'] . '"></span></a>';
		}
	}
...
function optionsframework_fields() {
	...
	foreach ( $options as $value ) {
		...
		// Wrap all options
		if ( ( $value['type'] != "heading" ) && ( $value['type'] != "info" ) && ( $value['type'] != "note" )) {
			...
			if ( isset( $value['name'] ) ) {
				$output .= '<h4 class="heading ' . $value_class . '">' . esc_html( $value['name'] ) . '</h4>' . "\n";
			}
			if ( $value['type'] != 'editor' ) {
				 $images_class = ' control-'.$value['type'].$value_class;
				//if ($value['type'] = 'editor')
				$output .= '<div class="option">' . "\n" . '<div class="controls'.$images_class.'">' . "\n";
			}
			else {
				$output .= '<div class="option">' . "\n" . '<div>' . "\n";
			}
		...
		// Slider
		case 'slider':
			$output .= '<input id="' . esc_attr( $value['id'] ) . '" class="of-input sslider" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" type="text" value="' . esc_attr( $val ) . '" data-slider="true" data-slider-range="'.$value['range'].'" data-slider-step="'.$value['step'].'" data-slider-snap="true" /> <span class="lblsldr" >' . esc_attr( $val ) . '</span>';           
			break;
		...
		case 'backup':
			...
			$output .= '<textarea id="export_data" class="of-input" rows="8">'.$backup_data . '</textarea>';
			$output .= '<a href="#" id="of_import_button" class="button" title="Restore Options">Update Options</a>';
		...
		case "tiles":
			...
			//$i++;
				...
				$output .= '<span>';
				$output .= '<input type="radio" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" class="checkbox of-radio-tile-radio" value="'. $key .'" '.$checked.' />';
				$output .= '<div title="' . $key . '" class="of-radio-tile-img '. $selected .'" style="background: url('.$option.')" onClick="document.getElementById(\''. esc_attr( $id ).'\').checked = true;$(\'#wpwrap\').css({background:\'url('.$option.') repeat;\'});"></div>';
				$output .= '</span>';				
			}
			break;
		...
		case "carticon":
			...
			//$i++;
				...
				$output .= '<span>';
				$output .= '<input type="radio" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" class="checkbox of-radio-tile-radio" value="'. $key .'" '.$checked.' />';
				$output .= '<div title="' . $key . '" class="of-radio-tile-img '. $selected .'" onClick="document.getElementById(\''. esc_attr( $id ).'\').checked = true;"><i class="flaticon-'.$option.'"> </i></div>';
				$output .= '</span>';				
			}
			break;
		// Image Selectors
		...
		// Image Selectors
		case "demo":
			$output .= '<a href="#" id="of_import_demo" class="button button-primary" title="Import Demo">Import Demo</a>';
			$name = $option_name .'['. $value['id'] .']';
			foreach ( $value['options'] as $key => $option ) {
				$selected = '';
				$checked = '';
				...
				$output .= '<div class="demoitem">';
				$output .= '<input type="radio" id="' . esc_attr( $value['id'] .'_'. $key) . '" class="of-radio-img-radio" value="' . esc_attr( $key ) . '" name="' . esc_attr( $name ) . '" '. $checked .' />';
				$output .= '<div class="of-radio-img-label">' . esc_html( $key ) . '</div>';
				$output .= '<img src="' . esc_url( $option ) . '" alt="' . $option .'" class="of-radio-img-img' . $selected .'" onclick="document.getElementById(\''. esc_attr($value['id'] .'_'. $key) .'\').checked=true;setDemo(\''. $key .'\');" />';
				$output .= '</div>';
				$output .= '<input type="hidden" id="demoselected" />';
			}
		...
		case "note":
			...
			$output .= '<div ' . $id . 'class="' . esc_attr( $class ) . '"><p>' . "\n";
			if ( $value['desc'] ) {
				$output .=  $value['desc']  . "\n";
			}
			$output .= '</p></div>' . "\n";
